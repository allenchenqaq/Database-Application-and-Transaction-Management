...
def getRenterID():
    ...
    cid = -1
    conn = get_db()
    cursor = conn.execute("select cid,status from Rental where status='open' and mid=?", mid)
    records = cursor.fetchall()
    if len(records) != 0:
        cid = records[0][0]
    response = {'cid': cid}
...
def getRemainingRentals():
    ...
    cursor = conn.execute("""select max_movies
                             from RentalPlan
                             join Customer on Customer.pid=RentalPlan.pid
                             where cid=?""", cid)
    records = cursor.fetchall()
    if len(records) != 0:
        max = records[0][0]
    # conn.execute("commit")
    # conn.execute("begin transaction")
    cursor2 = conn.execute("""select count(*)
                              from Rental
                              where status='open' and cid=?""", cid)
    records2 = cursor2.fetchall()
    if len(records2) != 0:
        remain = records2[0][0]
    conn.execute("commit")
    conn.autocommit = True
    response = {"remain": max - remain}
    return jsonify(response)
...
def rent():
    ...
    cursor3 = conn.execute("""select *
                              from Rental
                              where status='open' and mid=?""", mid)
    ...
    if c1 == True & c2 == True:
        res = "success"
        conn.execute("insert into Rental values(?,?,?,?)", (cid, mid, currentTime(), 'open'))
        conn.execute("commit")
    else:
